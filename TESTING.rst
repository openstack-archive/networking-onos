Testing Networking-onos + neutron
=================================

Overview
--------

The unit tests (networking-onos/test/unit/) are meant to cover as much code as
...
for DevStack at https://github.com/bcwaldon/vagrant_devstack.
 ...
 .. note::
    ...
    If you prefer not to use DevStack, you can still check out source code on
    your local machine and develop from there.
...
Tests written can also be debugged by adding pdb break points. Normally if you
add a break point and just run the tests with normal flags they will end up in
failing. There is debug flag you can use to run after adding pdb break points
in the tests.

Set break points in your test code and run::

    tox -e debug networking_onos.tests.unit.module.functionality
...
For example, the following would run only the ONOSL3PluginTestCase tests from
networking_onos/tests/unit/plugins/l3/test_driver.py ::

      $ tox -e py27 networking_onos.tests.unit.plugins.l3.ONOSL3PluginTestCase
...
And then run functional test as follows::
    ...
    # assuming devstack is setup with networking-onos
    $ cd networking-onos
