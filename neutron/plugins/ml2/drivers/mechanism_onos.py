...
ONOS_NETWORKS = 'networks'
ONOS_SUBNETS = 'subnets'
ONOS_PORTS = 'ports'
...
class ONOSAuthError(n_exc.NeutronException):
    message = '%(msg)s'
...
class JsessionId(requests.auth.AuthBase):
    ...
    def __init__(self, url, username, password):
        ...
        # data is returned from ONOS. This is not implemented in the Hydrogen
        # release of ONOS, but will be implemented in the Helium
        # timeframe. Hydrogen will silently ignore this value.
        self.url = str(url) + '/' + ONOS_NETWORKS + '?limit=1'
        self.username = username
        self.password = password
        self.auth_cookies = None
        self.last_request = None
        self.expired = None
        self.session_timeout = cfg.CONF.ml2_onos.session_timeout * 60
        self.session_deadline = 0

    def obtain_auth_cookies(self):
        """Make a REST call to obtain cookies for ONOS authenticiation."""

        try:
            r = requests.get(self.url, auth=(self.username, self.password))
            r.raise_for_status()
        except requests.exceptions.HTTPError as e:
            raise ONOSAuthError(msg="Failed to authenticate with "
                                        "ONOS: %s" % e)
        except requests.exceptions.Timeout as e:
            raise ONOSAuthError(msg="Authentication Timed Out: %s" % e)
...
class ONOSMechanismDriver(api.MechanismDriver):
    ...
    """Mechanism Driver for ONOS.

    This driver was a port from the Tail-F NCS MechanismDriver.  The API
    exposed by ONOS is slightly different from the API exposed by NCS,
    ...
    def create_network_postcommit(self, context):
        self.synchronize('create', ONOS_NETWORKS, context)

    def update_network_postcommit(self, context):
        self.synchronize('update', ONOS_NETWORKS, context)

    def delete_network_postcommit(self, context):
        self.synchronize('delete', ONOS_NETWORKS, context)

    def create_subnet_postcommit(self, context):
        self.synchronize('create', ONOS_SUBNETS, context)

    def update_subnet_postcommit(self, context):
        self.synchronize('update', ONOS_SUBNETS, context)

    def delete_subnet_postcommit(self, context):
        self.synchronize('delete', ONOS_SUBNETS, context)

    def create_port_postcommit(self, context):
        self.synchronize('create', ONOS_PORTS, context)

    def update_port_postcommit(self, context):
        self.synchronize('update', ONOS_PORTS, context)

    def delete_port_postcommit(self, context):
        self.synchronize('delete', ONOS_PORTS, context)

    def synchronize(self, operation, object_type, context):
        """Synchronize ONOS with Neutron following a configuration change."""
        if self.out_of_sync:
    ...
    def filter_create_port_attributes(cls, port, context):
        ...
        # TODO(kmestery): Converting to uppercase due to ONOS bug
        # https://bugs.onos.org/show_bug.cgi?id=477
        port['mac_address'] = port['mac_address'].upper()
        try_del(port, ['status'])

    def sync_resources(self, collection_name, context):
        """Sync objects from Neutron over to ONOS.

        This will handle syncing networks, subnets, and ports from Neutron to
        ONOS. It also filters out the requisite items which are not
    ...
    @utils.synchronized('onos-sync-full')
    def sync_full(self, context):
        """Resync the entire database to ONOS.
        ...
        if not self.out_of_sync:
            return
        for collection_name in [ONOS_NETWORKS, ONOS_SUBNETS, ONOS_PORTS]:
    ...
    def sync_single_resource(self, operation, object_type, context):
        """Sync over a single resource from Neutron to ONOS.

        Handle syncing a single operation over to ONOS, and correctly
    ...
    def sendjson(self, method, urlpath, obj):
        """Send json to the ONOS controller."""
...
ONOSMechanismDriver.create_object_map = {
    ONOS_NETWORKS: ONOSMechanismDriver.filter_create_network_attributes,
    ONOS_SUBNETS: ONOSMechanismDriver.filter_create_subnet_attributes,
    ONOS_PORTS: ONOSMechanismDriver.filter_create_port_attributes}

ONOSMechanismDriver.update_object_map = {
    ONOS_NETWORKS: ONOSMechanismDriver.filter_update_network_attributes,
    ONOS_SUBNETS: ONOSMechanismDriver.filter_update_subnet_attributes,
    ONOS_PORTS: ONOSMechanismDriver.filter_update_port_attributes}
