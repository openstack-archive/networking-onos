...
from neutron.plugins.ml2.drivers import mechanism_onos
...
class ONOSTestCase(test_plugin.NeutronDbPluginV2TestCase):
    ...
    def setUp(self):
        ...
        config.cfg.CONF.set_override('url', 'http://127.0.0.1:9999', 'ml2_onos')
        config.cfg.CONF.set_override('username', 'someuser', 'ml2_onos')
        config.cfg.CONF.set_override('password', 'somepass', 'ml2_onos')

        super(ONOSTestCase, self).setUp(PLUGIN_NAME)
        self.port_create_status = 'DOWN'
        self.segment = {'api.NETWORK_TYPE': ""}
        self.mech = mechanism_onos.ONOSMechanismDriver()
        mechanism_onos.ONOSMechanismDriver.sendjson = (
            self.check_sendjson)
...
class ONOSMechanismConfigTests(testlib_api.SqlTestCase):
    ...
    def _set_config(self, url='http://127.0.0.1:9999', username='someuser',
        ...
        config.cfg.CONF.set_override('mechanism_drivers',
                                     ['logger', 'onos'],
                                     'ml2')
        config.cfg.CONF.set_override('url', url, 'ml2_onos')
        config.cfg.CONF.set_override('username', username, 'ml2_onos')
        config.cfg.CONF.set_override('password', password, 'ml2_onos')
...
class ONOSMechanismTestBasicGet(test_plugin.TestBasicGet,
                                        ONOSTestCase):
    pass


class ONOSMechanismTestNetworksV2(test_plugin.TestNetworksV2,
                                          ONOSTestCase):
    pass


class ONOSMechanismTestSubnetsV2(test_plugin.TestSubnetsV2,
                                         ONOSTestCase):
    pass


class ONOSMechanismTestPortsV2(test_plugin.TestPortsV2,
                                       ONOSTestCase):
    pass
...
class AuthMatcher(object):
    ...
    def __eq__(self, obj):
        return (obj.username == config.cfg.CONF.ml2_onos.username and
                obj.password == config.cfg.CONF.ml2_onos.password)
...
class DataMatcher(object):
    ...
    def __init__(self, operation, object_type, context):
        ...
        filter_map = getattr(mechanism_onos.ONOSMechanismDriver,
...
class ONOSMechanismDriverTestCase(base.BaseTestCase):
    ...
    def _test_single_operation(self, method, context, status_code,
        ...
        mock_method.assert_called_once_with(
            ...
            timeout=config.cfg.CONF.ml2_onos.timeout, *args, **kwargs)
    ...
    def _test_create_resource_postcommit(self, object_type, status_code,
        ...
        url = '%s/%ss' % (config.cfg.CONF.ml2_onos.url, object_type)
        kwargs = {'url': url,
    ...
    def _test_update_resource_postcommit(self, object_type, status_code,
        ...
        context = self._get_mock_operation_context(object_type)
        url = '%s/%ss/%s' % (config.cfg.CONF.ml2_onos.url, object_type,
                             context.current['id'])
        kwargs = {'url': url,
